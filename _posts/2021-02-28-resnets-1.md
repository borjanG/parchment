...
The main practical challenge of ML in *training* the ResNet, namely finding the optimal parameters $\theta=${$w^k, b^k$} which minimize the (regularized) empirical risk (we focus on cross-entropy loss and weight-decay reg.)
